...
from mlpack.datasets.tokenization import is_punctuation, TokenizerWithAlignment

TOKENIZER = TokenizerWithAlignment()

LABELS = ["[PAD]", "O", "B-Gene", "I-Gene", "B-FamilyName", "I-FamilyName",
          "B-DomainMotif", "I-DomainMotif", "[CLS]", "[SEP]", "X"]
...
class InputExample:
    ...
    def __init__(self, id, orig_text, doc_tokens, labels, char_to_word_offset):
        self.id = id
        self.orig_text, self.doc_tokens, self.labels = orig_text, doc_tokens, labels
...
class InputFeatures(object):
    ...
    def __init__(self, id, input_ids, input_mask, segment_ids, label_id, valid_ids=None, label_mask=None, masked_word=None):
        self.id = id
...
def convert_entry_to_example(entry):
    example_id = entry[:entry.find('|')]
    ...
    doc_tokens, char_to_word_offset = TOKENIZER.tokenize(text)

    labels = ['O'] * len(doc_tokens)

    annotations = []
    for line in annotations_text:
        annotation_split = line.split('\t')[1:]
        start = annotation_split[0]
        end = annotation_split[1]
        label = annotation_split[3]
        annotations.append(Annotation(int(start), int(end), label))

    for annot in annotations:
        start_token = char_to_word_offset[annot.start]
    ...
    return InputExample(
        id=example_id,
        orig_text=text,
        doc_tokens=[tok.text for tok in doc_tokens],
        labels=labels,
...
def convert_example_to_spanfeatures(example, label_list, max_seq_length, tokenizer,
    ...
    all_doc_labels = []
    all_doc_labels_mask = []
    doc_tokens, char_to_word_offset = example.doc_tokens, example.char_to_word_offset
    word_to_char_offset = {
        v: [i for i, q in enumerate(char_to_word_offset) if q == v] for v in list(set(char_to_word_offset))
    }

    doc_labels = example.labels if training else ['[UNK]'] * len(doc_tokens)
    assert len(doc_labels) == len(doc_tokens)
...
def convert_examples_to_spanfeatures(examples, label_list, max_seq_length, tokenizer,
                                     doc_stride, training=False):
    return [
        convert_example_to_spanfeatures(example, label_list, max_seq_length, tokenizer,
                                        doc_stride, training) for example in examples
    ]
